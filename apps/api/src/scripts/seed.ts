...
(async () => {
  ...
  const studentUserId = "jackshanan95gmailcom-id"; // consistent with earlier logs
  ...
  // 4) Study Notes
  await StudyNote.deleteMany({}); // make the demo consistent
  ...
  // 5) Tutor Materials (public + student-specific)
  ...
  await TutorMaterial.insertMany([
    {
      ...
      is_active: true,
      student_ids: [], // public (our API will return these for everyone)
    },
    {
  ...
  // 7) Session bookings in the future (so pages render)

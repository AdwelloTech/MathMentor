...
const MONGODB_URI =
  process.env.MONGODB_URI || process.env.MONGO_URL || "mongodb+srv://adwello204_db_user:Adwello2025@cluster0.n7krmxe.mongodb.net/mathmentor?retryWrites=true&w=majority";
...
function sanitizeSort(sort: any, fallback: any) {
  if (!sort || typeof sort !== "object") return fallback;
  const allowed = ["display_name","name","sort_order","createdAt","updatedAt","title","grade"];
  const clean: any = {};
  for (const k of Object.keys(sort)) if (allowed.includes(k)) clean[k] = sort[k] === -1 ? -1 : 1;
  return Object.keys(clean).length ? clean : fallback;
}
function normalizeQuizDoc(doc: any) {
  if (!doc) return null;
  const id = String(doc._id ?? "");
  const raw = Array.isArray(doc.questions) ? doc.questions : [];
  const questions = raw.map((r: any, i: number) => {
    const question = r?.question ?? r?.q ?? r?.text ?? "";
    const options  = r?.options ?? r?.choices ?? r?.answers ?? [];
    const correctIndex =
      typeof r?.correctIndex === "number" ? r.correctIndex
      : typeof r?.correctAnswerIndex === "number" ? r.correctAnswerIndex
      : typeof r?.answerIndex === "number" ? r.answerIndex
      : -1;
    return {
      q: r?.q ?? question,
      question,
      options,
      choices: r?.choices ?? options,
      answers: r?.answers ?? options,
      correctIndex,
      correctAnswerIndex: correctIndex,
      answerIndex: correctIndex,
      explanation: r?.explanation ?? r?.explain ?? "",
      tags: Array.isArray(r?.tags) ? r.tags : [],
      idx: i,
    };
  });
  return { ...doc, id, questions, items: questions, questionCount: questions.length };
}

// ---------- subjects ----------
app.get("/api/subjects", async (req, res) => {
  const q = parseJSON<Record<string, any>>(req.query.q, {});
  const sort = sanitizeSort(parseJSON(req.query.sort, {}), { display_name: 1, sort_order: 1, createdAt: 1 });
  const limit = Math.min(Number(req.query.limit ?? 500), 1000);
  const docs = await mongoose.connection.db.collection("subjects")
    .find(q).sort(sort).limit(limit).toArray();
...
app.get("/api/quizzes", async (req, res) => {
  const q = parseJSON<Record<string, any>>(req.query.q, {});
  const sort = sanitizeSort(parseJSON(req.query.sort, {}), { createdAt: -1, title: 1 });
  const limit = Math.min(Number(req.query.limit ?? 100), 500);
  const docs = await mongoose.connection.db.collection("quizzes")
    .find(q).sort(sort).limit(limit).toArray();
  res.json(docs.map(normalizeQuizDoc));
});

// (optional) stats + detail
app.get("/api/quizzes/stats", async (_req, res) => {
  const coll = mongoose.connection.db.collection("quizzes");
  const [total, active] = await Promise.all([
    coll.countDocuments({}), coll.countDocuments({ is_active: true })
  ]);
...
app.get("/api/quizzes/:id", async (req, res) => {
  ...
  try {
    ...
    if (ObjectId.isValid(id)) doc = await coll.findOne({ _id: new ObjectId(id) });
  } catch {}
  if (!doc) doc = await coll.findOne({ slug: id });
  if (!doc) return res.status(404).json({ error: "Quiz not found" });
  res.json(normalizeQuizDoc(doc));
});

// Optional aliases (in case UI calls different paths)
app.get("/api/v1/quizzes", (req,res) => (app as any)._router.handle(req,res));
app.get("/api/v1/quizzes/:id", (req,res) => (app as any)._router.handle(req,res));

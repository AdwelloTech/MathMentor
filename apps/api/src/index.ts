...
function safeListen(app: any, primary = Number(process.env.PORT) || 8080, fallback = 8080) {
  ...
  server.on('error', (err: any) => {
    if (err && err.code === 'EADDRINUSE' && primary !== fallback) {
      console.warn(`[api] :${primary} in use, retrying :${fallback}`);
      setTimeout(() => {
        const s2 = app.listen(fallback, () => console.log(`[api] listening on :${fallback}`));
        s2.on('error', (e:any) => { console.error('[api] failed to bind', e); process.exit(1); });
      }, 200);
    } else {
...
// If file already calls app.listen, replace it with safeListen(app)
...
export type ClassType = InferSchemaType<typeof ClassTypeSchema>;

// Ensure we don’t re-register the model in dev hot-reloads
export const ClassTypeModel =
  mongoose.models.ClassType || mongoose.model<ClassType>("ClassType", ClassTypeSchema, "class_types");
  ...
  try {
    ...
    if (envEmail && emailLc === envEmail) {
      ...
      if (envPass.startsWith("$2"))
        ok = await bcrypt.compare(password, envPass);
    ...
    // 2) DB-based admin
    const adm = await AdminUser.findOne({
      email: emailLc,
      is_active: true,
    }).lean();
...
function pickLimitOffset(q: any) {
  const limit  = Math.max(0, Math.min(Number(q?.limit ?? 200), 1000));
  const offset = Math.max(0, Number(q?.offset ?? 0));
  return { limit, offset };
}

async function handleListClassTypes(req: any, res: any) {
  const q = parseJSON<Record<string, any>>(req.query?.q, {});
  const sort = parseJSON<Record<string, any>>(req.query?.sort, { sort_order: 1, createdAt: 1, name: 1 });
  const { limit, offset } = pickLimitOffset(req.query);
  const items = await ClassTypeModel.find(q).sort(sort).skip(offset).limit(limit).lean();
  return res.json({ ok: true, data: items });
}

// Aliases your frontend might use
app.get("/api/class_types", handleListClassTypes);     // snake_case (your client is using this)
app.get("/api/class-types", handleListClassTypes);     // kebab-case
app.get("/api/v1/class-types", handleListClassTypes);  // versioned alias
  ...
  const pipeline: any[] = [
    {
      $match: {
        is_active: true,
        ...(gradeFilter ? { grade_level_code: gradeFilter } : {}),
      },
    },
    {
      $lookup: {
...
app.delete("/api/profiles/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const filter = /^[0-9a-fA-F]{24}$/.test(id) ? { _id: id } : { user_id: id };
    const deleted = await Profile.findOneAndDelete(filter);
    if (!deleted) {
      return res.status(404).json({ ok: false, error: "Profile not found" });
    }
...
app.get("/api/quizzes/available", async (req, res) => {
  try {
    ...
    for (const a of attempts) {
      ...
      if (
        !attemptByQuiz.has(key) ||
        attemptByQuiz.get(key)?.status !== "completed"
      ) {
...
app.post("/api/quizzes/:id/attempts", async (req, res) => {
  try {
    ...
    if (!student_id)
      return res.status(400).json({ ok: false, error: "student_id required" });
    ...
    res.json({
      ok: true,
      data: { id: String(attempt._id), quiz_id: id, status: attempt.status },
    });
  } catch (e: any) {
    res.status(400).json({ ok: false, error: e.message });
  }
...
app.get("/api/note_subjects", async (req, res) => {
  const q = parseJSON<any>(req.query.q as string) || { is_active: true };
  const sort = parseJSON<any>(req.query.sort as string) || {
    sort_order: 1,
    name: 1,
  };
  const limit = req.query.limit ? Number(req.query.limit) : 200;
...
app.get("/api/study-notes/search", async (req, res) => {
  try {
    const search_term =
      (req.query.q as string) || (req.query.search_term as string) || "";
    const subject_filter =
      (req.query.subject as string) ||
      (req.query.subject_filter as string) ||
      "";
    const grade_filter =
      (req.query.grade as string) || (req.query.grade_filter as string) || "";
...
app.post("/api/increment_tutor_note_download_count", async (req, res) => {
  try {
    ...
    if (!note_id)
      return res.status(400).json({ ok: false, error: "note_id required" });
...
app.get("/api/classes", async (req, res) => {
  ...
  const limit = req.query.limit ? Number(req.query.limit) : 100;
  const offset = req.query.offset ? Number(req.query.offset) : 0;
  // For now, return empty array since we don't have a Classes model yet
  res.json({ ok: true, data: [], items: [], total: 0, limit, offset });
});
...
app.get("/api/check_premium_access", async (req, res) => {
  try {
    ...
    const premiumUntil = prof?.premium_until
...
app.get("/api/admin/dashboard/summary", async (req, res) => {
  try {
    ...
    const newUsers7 = await Profile.countDocuments({
      createdAt: { $gte: last7 },
    });
    ...
    res.json({
      totals: {
        ...
        tutor_applications_pending: 0, // TODO: implement when tutor applications are added
      },
      last_7_days: {
        new_users: newUsers7,
      },

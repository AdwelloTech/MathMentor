...
class AdminQuizServiceClass {
  ...
  async getAllQuizzes(): Promise<AdminQuiz[]> {
    try {
      ...
      // Handle different response formats - some APIs return 'items', others return 'value' or direct array
      const quizzes = response.data?.items || response.data?.value || response.data || [];
      console.log('Raw quizzes response:', response.data);
      console.log('Processed quizzes array:', quizzes);
      
      // For each quiz, get tutor information and attempt statistics
      const quizzesWithDetails = await Promise.all(
        quizzes.map(async (quiz: any) => {
          try {
            // Get tutor profile (only if tutor_id exists)
            let tutor = null;
            if (quiz.tutor_id) {
              try {
                ...
                tutor = tutorResponse.data?.items?.[0] || null;
              } catch (error) {
                console.warn('Error fetching tutor for quiz:', quiz.id, error);
                tutor = null;
              }
            }
  ...
  async getQuizDetails(quizId: string): Promise<AdminQuiz | null> {
    try {
      console.log('getQuizDetails called with quizId:', quizId);
      
      // Since the MongoDB query doesn't work reliably, let's fetch all quizzes and filter in JavaScript
      const quizResponse = await this.api.get('/api/quizzes');
      console.log('All quizzes response:', quizResponse.data);
      
      const allQuizzes = quizResponse.data?.items || quizResponse.data?.value || quizResponse.data || [];
      console.log('All quizzes array:', allQuizzes);
      
      // Find the quiz by id or _id
      const quiz = allQuizzes.find((q: any) => 
  ...
  async getQuizStats(): Promise<QuizStats> {
    try {
      ...
      const quizzes = quizzesResponse.data?.items || quizzesResponse.data?.value || quizzesResponse.data || [];
  ...
  async getQuizPdfs(): Promise<any[]> {
    try {
      const response = await this.api.get('/api/quiz_pdfs');
      return response.data?.items || [];
    } catch (error) {
      console.error('Error in getQuizPdfs:', error);
      return [];
    }
  }
}

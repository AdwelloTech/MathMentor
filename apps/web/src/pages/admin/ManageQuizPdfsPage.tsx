import AdminQuizService from "@/lib/adminQuizService";
...
/** ---------- helpers to prevent "is not iterable" crashes ---------- */
function toArray<T = any>(val: any, fallback: T[] = []): T[] {
  if (Array.isArray(val)) return val as T[];
  if (val && Array.isArray((val as any).items)) return (val as any).items as T[];
  return fallback;
}
...
const ManageQuizPdfsPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setPdfs(toArray<QuizPdf>(pdfsData));
      setGradeLevels(toArray<GradeLevel>(gradeLevelsData));
      setSubjects(toArray<Subject>(subjectsData));
    } catch (error) {
  ...
  const filterPdfs = () => {
    const list = toArray<QuizPdf>(pdfs);
    let filtered = [...list];

    const term = searchTerm.trim().toLowerCase();
    if (term) {
      filtered = filtered.filter((pdf) =>
        str(pdf.file_name).toLowerCase().includes(term)
      );
    }

    if (filterGrade !== "all") {
      filtered = filtered.filter((pdf) => str(pdf.grade_level_id) === str(filterGrade));
    }

    if (filterSubject !== "all") {
      filtered = filtered.filter((pdf) => str(pdf.subject_id) === str(filterSubject));
    }

    if (filterStatus !== "all") {
      const isActive = filterStatus === "active";
      filtered = filtered.filter((pdf) => Boolean(pdf.is_active) === isActive);
    }
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      // Upload to AI helper (supports two shapes)
      const result = await uploadPdfForAI(uploadForm.file);
      let pdfBase64: string, fileName: string, fileSize: number;

      if (result && Array.isArray((result as any).pdfs) && (result as any).pdfs.length > 0) {
        ({ pdfBase64, fileName, fileSize } = (result as any).pdfs[0]);
      } else {
        ({ pdfBase64, fileName, fileSize } = result as any);
      }

      const payload: any = {
        file_name: uploadForm.file_name || uploadForm.file.name || fileName || "Untitled.pdf",
        file_path: pdfBase64,
        file_size: fileSize,
      ...
      if (uploadForm.grade_level_id && uploadForm.grade_level_id !== "") {
        payload.grade_level_id = uploadForm.grade_level_id;
      }

      const newPdf = await quizPdfService.create(payload);
      setPdfs((prev) => [newPdf, ...toArray(prev)]);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setPdfs((prev) => toArray(prev).map((pdf) => (pdf.id === selectedPdf.id ? updatedPdf : pdf)));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setPdfs((prev) => toArray(prev).filter((pdf) => pdf.id !== deletingId));
  ...
  const handleToggleActive = async (pdf: QuizPdf) => {
    try {
      ...
      setPdfs((prev) => toArray(prev).map((p) => (p.id === pdf.id ? updatedPdf : p)));
      toast.success(`PDF ${updatedPdf.is_active ? "activated" : "deactivated"} successfully`);
    } catch (error) {
  ...
  const openEditModal = (pdf: QuizPdf) => {
    setSelectedPdf(pdf);
    setEditForm({
      file_name: str(pdf.file_name),
      grade_level_id: pdf.grade_level_id,
      subject_id: str(pdf.subject_id),
      is_active: Boolean(pdf.is_active),
    });
  ...
  const getGradeDisplayName = (gradeId: string): string => {
    const grade = toArray(gradeLevels).find((g) => str(g.id) === str(gradeId));
    return grade?.display_name || "Unknown Grade";
  };

  const getSubjectDisplayName = (subjectId: string): string => {
    const subject = toArray(subjects).find((s) => str(s.id) === str(subjectId));
    return subject?.display_name || "Unknown Subject";
  };

  if (loading && toArray(pdfs).length === 0) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Manage Quiz PDFs</h1>
          <p className="text-gray-600">Upload and manage PDFs that students can use for AI quiz generation</p>
        </div>

        {/* Controls */}
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <Button onClick={() => setShowUploadModal(true)} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {toArray(filteredPdfs).map((pdf) => {
            const uploadedAt = (pdf as any).created_at || (pdf as any).createdAt || Date.now();
            return (
              <motion.div
                key={String(pdf.id)}
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-900 text-lg">
                          {str(pdf.file_name) || "Untitled.pdf"}
                        </h3>
                        <p className="text-sm text-gray-500">
                          {formatFileSize((pdf as any).file_size)}
                        </p>
                      </div>
                    </div>
                    <Badge variant={pdf.is_active ? "default" : "secondary"} className="ml-2">
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      {getGradeDisplayName(str(pdf.grade_level_id))}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      <BookOpenIcon className="w-4 h-4" />
                      {getSubjectDisplayName(str(pdf.subject_id))}
                    </div>
        ...
        {toArray(filteredPdfs).length === 0 && !loading && (

...
import classTypesService from "@/lib/classTypesService";
...
import tutorCompatServices from "@/lib/tutorCompatServices";
...
async function createClassCompat(payload: any) {
  // Prefer existing service if available
  try {
    if ((classSchedulingService as any)?.classes?.create) {
      return await (classSchedulingService as any).classes.create(payload);
    }
  } catch {}

  // Fallbacks across common API bases
  for (const base of API_BASES) {
    const res = await tryFetchJSON(base + "/api/classes", {
      method: "POST",
  ...
  const loadData = async () => {
    ...
    try {
      ...
      let subs: any[] = [];
      try {
        types = await (classSchedulingService as any).classTypes.getAll();
      } catch {
        types = await tutorCompatServices.classTypes.getAll();
      }
      try {
        classes = await (classSchedulingService as any).classes.getByTutorId(user.id);
      } catch {
        classes = await tutorCompatServices.classes.getAll(user.id);
      }
      try {
        subs = await subjectsService.listActive();
      } catch {
        subs = [];
      }
  ...
  const handleCreateClass = async () => {
    ...
    try {
      ...
      await createClassCompat(classData);

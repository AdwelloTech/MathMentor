...
// allow tutor access if any tutor flags/role are present
function canSeeTutorMenusLocal(user: any, profile: any): boolean {
  const isTutorRole = user?.role === "tutor" || profile?.role === "tutor";
  const hasFlags = Boolean(
    profile?.is_tutor === true ||
    profile?.tutor?.status === "approved" ||
    profile?.features?.tutor === true ||
    profile?.features?.tutor_dashboard === true ||
    profile?.features?.tutor_subscription === true
  );
  return isTutorRole || hasFlags;
}
...
function ProtectedRoute({ children, requiredRole }: ProtectedRouteProps) {
  ...
  // Special case: unlock tutor routes if tutor role OR tutor feature flags are present
  if (requiredRole === "tutor") {
    if (!canSeeTutorMenusLocal(user, profile)) {
      return <Navigate to="/unauthorized" replace />;
    }
    return <>{children}</>;
  }

...
export const classSchedulingService = {
  ...
  // Tutor Classes
  classes: {
    create: async (classData: CreateClassFormData & { tutor_id: string }): Promise<TutorClass> => {
      // Get class type details
      const classType = await classSchedulingService.classTypes.getById(classData.class_type_id);
      
      // Calculate end time based on duration
      const startTime = new Date(`2000-01-01T${classData.start_time}`);
      const endTime = new Date(startTime.getTime() + classType.duration_minutes * 60000);
    ...
    getByTutorId: async (tutorId: string, filters?: ClassFilters): Promise<TutorClass[]> => {
      let query = supabase
        .from('tutor_classes')
        .select(`
          *,
    ...
    update: async (id: string, updates: UpdateClassFormData): Promise<TutorClass> => {
      const { data, error } = await supabase
        .from('tutor_classes')
        .update(updates)
        .eq('id', id)
    ...
    cancel: async (id: string): Promise<TutorClass> => {
      return await classSchedulingService.classes.update(id, { status: 'cancelled' });
    },

    // Get available classes for students to book
    getAvailableClasses: async (filters?: ClassSearchFilters): Promise<ClassSearchResult[]> => {
      let query = supabase
        .from('tutor_classes')
        .select(`
          *,
      ...
      const results: ClassSearchResult[] = await Promise.all(
        data.map(async (classRecord) => {
          ...
          return {
            ...
            available_slots: classRecord.max_students - classRecord.current_students,
  ...
  // Class Bookings
  bookings: {
    create: async (classId: string, studentId: string, paymentAmount: number): Promise<ClassBooking> => {
    ...
    getByStudentId: async (studentId: string, filters?: BookingFilters): Promise<ClassBooking[]> => {
      let query = supabase
        .from('class_bookings')
        .select(`
          *,
    ...
    update: async (id: string, updates: Partial<ClassBooking>): Promise<ClassBooking> => {
      const { data, error } = await supabase
        .from('class_bookings')
        .update(updates)
        .eq('id', id)
  ...
  availability: {
    ...
    update: async (tutorId: string, availability: AvailabilityFormData[]): Promise<TutorAvailability[]> => {
  ...
  stats: {
    getTutorStats: async (tutorId: string): Promise<TutorDashboardStats> => {
      ...
      const totalEarnings = classes
        ...
        .reduce((sum, c) => sum + (c.price_per_session * c.current_students), 0) || 0;
    ...
    getStudentStats: async (studentId: string): Promise<StudentDashboardStats> => {
      ...
      const upcomingBookings = bookings?.filter(b => b.booking_status === 'confirmed').length || 0;
      ...
      const totalTutors = new Set(uniqueTutors?.map((b: any) => b.class?.tutor_id).filter(Boolean)).size;
  ...
  zoom: {
    ...
    generateMeeting: async (classId: string, title: string, startTime: string): Promise<any> => {

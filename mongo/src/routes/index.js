...
/*
router.get('/instant_requests', (_req, res) => {
  res.json({ message: 'Welcome to the MathMentor API' });
})
*/
...
// Use your real Mongoose model
// const { TutorNotes } = require('../models/TutorNote');
...
function pickLimitOffset(q) {
  const limit  = Math.max(0, Math.min(Number(q?.limit ?? 100), 1000));
  const offset = Math.max(0, Number(q?.offset ?? 0));
  return { limit, offset };
}

router.get('/tutor_notes', async (req, res, next) => {
  try {
    const { q: qRaw, subjectId, is_active, sort: sortRaw } = req.query;
    console.log({ qRaw, subjectId, is_active, sortRaw },'--- IGNORE ---');
    // 1) Build filter
    const filter = parseJSON(qRaw, {}); // if q=JSON, merge it
    if (typeof qRaw === 'string' && qRaw.trim() && !qRaw.trim().startsWith('{')) {
      // q is a plain search string → search title/desc/content
      const rx = new RegExp(escapeRegExp(qRaw.trim()), 'i');
      filter.$or = [{ title: rx }, { description: rx }, { content: rx }];
    }
    if (subjectId) filter.subjectId = String(subjectId);
    if (typeof is_active !== 'undefined') {
      filter.is_active = String(is_active) === 'true' || String(is_active) === '1';
    }

    // 2) Sort, limit, offset
    const sort = parseJSON(sortRaw, { createdAt: -1 });
    const { limit, offset } = pickLimitOffset(req.query);

    // 3) Query DB
    const items = await (TutorNotes
      ? TutorNotes.find(filter).sort(sort).skip(offset).limit(limit).lean()
      : Promise.resolve([]));

    // 4) Respond (array or {ok,data} — choose the shape your client expects)
    // If your client unwraps arrays directly:
    res.json(items);

    // If your client expects { ok, data }:
